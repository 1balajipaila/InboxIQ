...
"""
InboxIQ Gmail integration.

Local development implementation:
- OAuth 2.0 with Google's installed-app flow
- Gmail API modify scope
- Polls unread messages in the Gmail Inbox
- Sends each message through the existing InboxIQ run_pipeline()
- Creates/uses InboxIQ and InboxIQ/Spam labels
- Saves user preferences locally in preferences.json

This module deliberately does NOT store a Gmail password.
Google OAuth creates token.json after the user authorizes access.
"""
...
def process_message(message, user_interests):
    ...
    if result["final_class"] == "INBOXIQ":
        _modify_message(
            email["id"],
            add_labels=[labels[LABEL_INBOXIQ]],
            remove_labels=["INBOX"],
        )
